...
const int LDR = A0; // Pin sensor del LDR.
// Variables
int tension; // Señal del LDR.
float valorLDR; // Resistencia aproximada del LDR.
float lux; // Intensidad de luz.
// Constantes
const float R = 1.0; // Valor de la resistencia en serie en kilos.

void setup() {
  Serial.begin(57600); // Comunicacion serie a 9600 baudios.
...
void loop() {
  ...
  tension = analogRead(LDR); // Lectura de la señal.
  valorLDR = ( (1023 * R) / tension) - R;// Encuentra resistencia de LDR.
  //lux = 800.69 * pow(valorLDR, -1.283); //Calcula intensidad luminosa en lx
  // Envia informacion por serie al monitor serial
  lux = 10.0*pow(valorLDR/5.0, -1.3); //Calcula intensidad luminosa en lx
  Serial.println(lux);
  
  PUBLICAR.publish(lux);
  delay(5000);
